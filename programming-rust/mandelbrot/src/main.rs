...
fn main() {
    ...
    if args.len() != 5 {
        writeln!(std::io::stderr(),
                 ...
                 Example: {} mandel.png 1000x750 -1.20,0.35 -1.0,0.20"#, args[0]).unwrap();
        std::process::exit(1);
    }
...
/// Try to determine whether c is in the Mandelbrot set.
/// 
/// if c is not a member, return Some(i), where i is the number of iteration
/// it took for c to leave the circle of radius. If c is a member, return None.
...
fn test_parse_pair() {
    ...
    assert_eq!(parse_pair::<f64>("1.5x0.5", 'x'), Some((1.5, 0.5)));
}
...
fn test_parse_complex() {
    assert_eq!(parse_complex("1.5, 0.5"), Some(Complex::<f64> { re: 1.5, im: 0.5}));
